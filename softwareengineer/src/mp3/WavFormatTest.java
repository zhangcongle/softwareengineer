...
public class WavFormatTest {
	// bestinshow-willingtolearn.wav
	public static void main(String[] args) {
		readAndRewriteWav();
	}

	private static void readAndRewriteWav() {
		String filePathToReadFromMinusExtension = "a";
		...
		String filePathToWriteTo = filePathToReadFromMinusExtension
				+ "-ReadThenWritten" + fileExtension;
...
class SamplingInfo {
	...
	public SamplingInfo(String name, int chunkSize, short formatCode,
			short numberOfChannels, int samplesPerSecond, short bitsPerSample) {
		this.name = name;
	...
	public static class Instances {
		public static SamplingInfo Default = new SamplingInfo("Default", 16, // chunkSize
				(short) 1, // formatCode
	...
	public Sample samplePrototype() {
		...
		if (this.bitsPerSample == 16) {
			returnValue = new Sample16((short) 0);
	...
	public String toString() {
		String returnValue = "<SamplingInfo " + "chunkSize='" + this.chunkSize
...
class WavFile {
	...
	public static WavFile readFromFilePath(String filePathToReadFrom) {
		...
		try {
			...
			SamplingInfo samplingInfo = new SamplingInfo("[from file]",
					chunkSize, formatCode, numberOfChannels, samplesPerSecond,
					bitsPerSample);

			Sample[][] samplesForChannels = Sample.buildManyFromBytes(
					samplingInfo, samplesForChannelsMixedAsBytes);

			returnValue = new WavFile(filePathToReadFrom, samplingInfo,
					samplesForChannels);
	...
	public void writeToFilePath() {
		try {
			int subchunk2Size = (int) (this.samplesForChannels[0].length
